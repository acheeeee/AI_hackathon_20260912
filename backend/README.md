...
目前測試結果為 71 passed，`caseapi` 總覆蓋率 94%，其中 `EvidenceRepository` 87%。資料庫預設寫入 `data/caseapi.db`，未納版控；用 `CASEAPI_DB_PATH` 可改位置。測試依賴在 `requirements-dev.txt`。
...
這只是可驗證的內部讀取／檢索層，尚未寫入 `evidence_records`，也沒有 run、工具活動、聊天端點或前端來源卡。實作進度與已知缺口見 [協作設計 05 §6](../docs/協作設計/05-實作順序與驗收.md)；接手實作先讀 [協作設計 06 交接](../docs/協作設計/06-交接與下一步.md)。
