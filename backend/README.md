...
scripts/run_api.sh    # 啟動 API；會載入專案根目錄的 .env 並印出實際 provider
```

**啟動用 `scripts/run_api.sh`，不要直接跑 `uvicorn caseapi.main:app`。**`config.py`
只讀 `os.environ`、不會自己讀 `.env`（刻意如此，否則 `test_model_provider_selection.py`
會被真實 `.env` 污染），直接跑 uvicorn 會讓 `.env` 的 `CASEAPI_MODEL_PROVIDER=agentcore`
失效、默默退回固定假模型，「問 AI」只剩佔位字串。腳本用 uvicorn 的 `--env-file` 載入。
