...
**2026-09-13 現況：新前端首頁（案件列表＋上傳＋原檔展開）已接上新版案件 API；新版案件 API、r3/BM25 證據層、run／事件、Evidence 工具、固定假模型端到端流程、線上 AWS Bedrock AgentCore provider，以及上傳建案（訴願書規則式抽欄位＋案件列表狀態）都已完成並用真實請求、真實樣本或真人瀏覽器操作驗證過。舊的五步驟精靈搬到 `/legacy-demo`，仍走舊後端 `/api/*`，兩邊不共用狀態。**
