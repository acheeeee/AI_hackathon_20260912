...
**2026-09-12 現況：既有 Vue＋舊版 API 可跑離線示範；新版案件 API、r3/BM25 證據層、run／事件、Evidence 工具與固定假模型端到端流程已完成。線上模型與現有 Vue 尚未接上。**
...
- 新 `/api/v1` 的案件隔離、SQLite、版本、提案／diff／採用已完成階段 A；B0／B1／B2 已完成 r3 BM25、run 與工具。B3 已加入案件內對話、固定 provider、背景 runner、assistant message、evidence GET 與 Last-Event-ID SSE replay；實測可經真實 r3 完成 search→open→answer。線上模型、長連線 SSE worker 與前端串接仍未完成。
