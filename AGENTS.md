...
Vitest is configured; the first committed frontend tests live under `src/utils/__tests__/*.spec.ts`. After the fixed-model B3 flow (including the cancel-race fix), the online AWS Bedrock AgentCore provider, upload intake with rule-based appeal-letter extraction, the AI chat sidebar, the appeal-deadline procedural review, statute selection (real BM25 search over r3 plus a saved pick list), and draft generation (the `draft` intent, which turns the selected statutes into a real `replace_document` proposal instead of a chat answer), the backend suite has 170 passing tests with 96% aggregate `caseapi` coverage; `draft_composition`, `draft_context` and the draft-generation route are at 100%, `draft_generation_service` at 96%, `draft_runner` at 93%, the chat runner at 98%, `agentcore_provider` at 100%, `appeal_extraction` at 94%, `EvidenceToolAdapter` at 95%, and `EvidenceRepository` at 87%; the frontend suite has 6 passing tests. `TestClient` alone missed a real cross-thread SQLite bug that only showed up under an actually-running `uvicorn` server — see 協作設計 06 §3 rule 12 — so `pytest` passing is not sufficient proof any endpoint the frontend calls actually works; verify live too. The preprocessing release validator passes 15/15 for r3 and the four-document layout regression passes 4/4. Legal-logic changes require source-based regression cases; passing transport/data-contract tests or showing a legal label is not evidence of legal correctness.
