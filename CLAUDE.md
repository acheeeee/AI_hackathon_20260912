...
目前已完成 Stage A、r3/BM25 證據底座、B1 run 持久化、B2 Evidence 工具 adapter 與 B3 固定假模型端到端。下一階段接線上模型 provider，必須沿用 `ModelProvider`／`ToolGateway` 邊界，不繞過案件隔離或證據 adapter。不要覆寫 r1/r2/r3、舊 KB 或原 PDF；前處理實驗使用新 release ID。不要碰未追蹤的 `.claude/launch.json`。憑證只能從環境讀取，不得寫入 DB、Git、文件或 log。
