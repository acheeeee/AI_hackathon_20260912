...
目前已完成 Stage A、r3/BM25 證據底座、B1 run 持久化與 B2 Evidence 工具 adapter。下一階段用固定假模型完成端到端。不要覆寫 r1/r2/r3、舊 KB 或原 PDF；前處理實驗使用新 release ID。不要碰未追蹤的 `.claude/launch.json`。線上模型雖已在後續順序中獲授權，仍要等固定假模型端到端通過，並且不得把憑證寫入 DB、Git、文件或 log。
